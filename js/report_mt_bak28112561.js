       ...
       $.each(rep.listOrder,function(keys,values){
         //console.log(values);
            ...
            if(values['FApprove'] != null && values['FApproveManager_status'] != null){
                $('#manager_signature').html('<img src=\"'+values['manager_signature']+'\" class=\"img-fluid\" width=\"100px\">');//ลายเช็นผู้จัดการ
        ...
        $.each(rep.listOrderType,function(orderTypeKeys,orderTypeValues){
            ...
            orderTable += " <div class=\"col-md-4 col-6\" >&nbsp;"+ orderTypeValues['FPrice'] +"&nbsp;บาท</div>";
            orderTable +="</div></div>";
       })

       SumAll = parseFloat(TotalAll) + parseFloat(unitPrice);

       $('#TotalAmt').text(SumAll.toLocaleString(undefined,{minimumFractionDigits: 2, maximumFractionDigits: 2}));//รวมเป็นเงินจำนวน
