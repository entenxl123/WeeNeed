$(function(){
       ...
       $.each(rep.listOrder,function(keys,values){
           //console.log(values);
            ...
            if(values['FApprove'] != null && values['FApproveManager_status'] != null && values['FApprove'] !="N"){
                $('#manager_signature').attr("src",chk_locatSign(values['manager_signature']));//ลายเช็นผู้จัดการ
            ...
            if(values['FApprove_super'] != null  && values['FApproveSuper_status'] !=null && values['FApprove_super'] !="N"){
                $('#super_signature').attr("src",chk_locatSign(values['super_signature']));//ลายเซ็นผู้อำนวยการ
    ...
    $.getJSON('../../api/controller/isd./php?status=ReadProgress&FRequestID='+$('#FRequestID').val(),function(rep){
        var table_progress = "";
        $.each(rep,function(index,items){
            table_progress += "<tr>";
                table_progress += "<td align='left' width='40%'>"+items.progress_detail+"</td>";
                table_progress += "<td width='20%'>"+items.progress_date+"</td>";
                table_progress += "<td width='20%'>"+items.emp_name+"</td>";
            table_progress += "</tr>";
        })

        $('#table-progress tbody').html(table_progress);
    })
